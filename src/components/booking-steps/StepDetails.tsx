...
export default function StepDetails({ bookingData, onContinue }: StepDetailsProps) {
  ...
  return (
    <div className="flex flex-col h-full flex-1">
      <div className="flex-1 flex flex-col gap-8 py-2">
        {details.map((player, index) => (
          <div key={index} className="flex flex-col gap-5">
            <h3 className="text-white text-[18px] font-semibold tracking-[-0.03em]">
      ...
      {/* ── CTA ───────────────────────────────────────── */}
      <div className="pt-6 border-t border-[rgba(255,255,255,0.06)] mt-auto pb-2 shrink-0 bg-[#0d0d0d] sticky bottom-0 z-10">
        <button
          type="button"
